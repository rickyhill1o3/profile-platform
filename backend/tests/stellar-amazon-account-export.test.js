...
const csv = buildStellarAmazonAccountsCsv([
    {
...
assert.strictEqual(
    csv,
    'email;password;region;authenticatorKey;accountType;cvv;loginIp;loginMethod\r\n' +
    'amazon-user@example.com;"pass;word";US;JBSWY3DPEHPK3PXP;;123;;\r\n'
);
...
assert.match(serverSource, /app\.get\("\/admin\/export\/stellar-amazon-accounts-csv"/);
...
assert.match(serverSource, /buildStellarAmazonAccountsCsv\(rows\)/);
assert.match(frontendSource, /'\/admin\/export\/stellar-amazon-accounts-csv\?'/);
assert.match(frontendSource, /stellar-accounts-amazon-active-/);
assert.match(adminHtml, /Export Active Stellar Amazon Accounts/);
assert.match(adminHtml, /script\.js\?v=20260925-stellar-amazon-accounts/);
