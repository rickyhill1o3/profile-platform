...
function parseRetailEmail(store,status,subject,text){
  ...
  const orderNumber = store==='supreme'
    ...
    : store==='pokemoncenter' || store==='pokemon'
      ...
      : store==='boxlunch'
        ? ((hay.match(/\bOrder\s*(?:Number|No\.?|ID|#)?\s*[:#-]?\s*(DL[A-Z0-9-]{5,30})\b/i)||hay.match(/\b(DL[A-Z0-9-]{5,30})\b/i)||[])[1]||'')
        : ['shopifyhazbinhotel','shopifytaylorswift'].includes(store)
          ? ((hay.match(/\bOrder\s*(?:Number|No\.?|ID|#)?\s*[:#-]?\s*#?([A-Z0-9-]{4,30})\b/i)||[])[1]||'')
