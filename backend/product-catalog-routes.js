    ...
    try {
      ...
      try {
        let countdownQuery = supabase
          .from('user_selected_countdowns')
          .select('user_id, created_at, users!inner(id, email, owner_admin_id), drop_countdowns!inner(id, site, label, scheduled_for)')
          .order('created_at', { ascending: false });
        ...
        const { data: countdownRows, error: countdownError } = await countdownQuery;
        if (countdownError && !String(countdownError.message || '').toLowerCase().includes('user_selected_countdowns')) {
          return res.status(500).json({ error: countdownError.message });
        }
        countdownUsers = (countdownRows || []).map((row) => ({
          preference_id: null,
          user_id: row.user_id,
          user_email: row.users?.email || '',
          owner_admin_id: row.users?.owner_admin_id || null,
          ...
          sku: '',
          product_name: row.drop_countdowns?.label || row.drop_countdowns?.site || '',
          image_url: '',
          run_mode: 'countdown',
          max_price: null,
          default_max_price: null,
      ...
      [...rows, ...countdownUsers].forEach((row) => {
        ...
        if (rowTime > prevTime) merged.set(row.user_id, row);
      });

      let items = [...merged.values()];
      if (search) {
        const q = search.toLowerCase();
        items = items.filter((row) => (row.user_email || '').toLowerCase().includes(q) || (row.product_name || '').toLowerCase().includes(q) || (row.site || '').toLowerCase().includes(q));
      }
      res.json({ items });
    } catch (err) {
      res.status(500).json({ error: err.message });
