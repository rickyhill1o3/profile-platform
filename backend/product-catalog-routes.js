const SUPPORTED_SITES = new Set(["amazon", "target", "walmart", "supreme", "pokemon", "general"]);
const PLACEHOLDER_DEFAULTS = {
  amazon: { sku: "AMAZON-NEXT-RELEASE", product_name: "Run Next Amazon Release", brand: "Amazon" },
  target: { sku: "TARGET-NEXT-RELEASE", product_name: "Run Next Target Release", brand: "Target" },
  walmart: { sku: "WALMART-NEXT-RELEASE", product_name: "Run Next Walmart Release", brand: "Walmart" },
  supreme: { sku: "SUPREME-NEXT-RELEASE", product_name: "Run Next Supreme Release", brand: "Supreme" },
  pokemon: { sku: "POKEMON-NEXT-RELEASE", product_name: "Run Next Pokémon Center Release", brand: "Pokémon Center" },
  general: { sku: "GENERAL-NEXT-RELEASE", product_name: "Run Next General Release", brand: "General" }
};
...
async function ensureActiveCatalogAndPlaceholder(supabase, site) {
  ...
  if (!activeCatalog?.id) {
    const { data: createdCatalog, error: createCatalogError } = await supabase
      .from("product_catalogs")
      .insert({ site, name: `${site} catalog`, is_active: true, export_date: new Date().toISOString() })
      .select("id, site, name, export_date")
      .single();
  ...
  const cfg = PLACEHOLDER_DEFAULTS[site];
  if (cfg) {
    const { data: existingPlaceholder, error: existingError } = await supabase
      .from("catalog_products")
      .select("id")
      .eq("catalog_id", activeCatalog.id)
      .or(`sku.eq.${cfg.sku},product_name.eq.${cfg.product_name}`)
      .limit(1)
      .maybeSingle();
    ...
    if (!existingPlaceholder?.id) {
      const { error: createProductError } = await supabase
        ...
        .insert({
          ...
          is_enabled: true,
          metadata: { virtual: true, release_type: "next_release" }
        });
...
module.exports = function registerProductCatalogRoutes({
  ...
  app.get("/product-catalog", auth, async (req, res) => {
    try {
      ...
      const activeCatalog = await ensureActiveCatalogAndPlaceholder(supabase, site);
      ...
      let query = supabase
        ...
        .order("release_mode_default", { ascending: false })
