...
// ===== PRODUCT PREFERENCE COMPAT ROUTES =====
app.get('/product-preferences', auth, async (req, res) => {
  try {
    ...
    const prefMap = new Map((prefs || []).map(p => [p.catalog_product_id, p]));

    const merged = (products || []).map(p => ({
      ...p,
      selected: !!prefMap.get(p.id)?.selected,
      run_mode: prefMap.get(p.id)?.run_mode || p.release_mode_default || 'live',
      max_price: prefMap.get(p.id)?.max_price || p.default_max_price
    }));
  ...
  try {
    ...
    for (const pref of preferences) {
      await supabase.from('user_product_selections').upsert({
        user_id: req.user_id,
        catalog_product_id: pref.product_id,
        selected: !!pref.selected,
        run_mode: pref.run_mode || 'live',
        max_price: pref.max_price || null,
        updated_at: new Date().toISOString()
...
app.get('/admin/product-preferences', auth, admin, async (req, res) => {
  try {
    ...
    const { data: prefs } = await supabase.from('user_product_selections').select('*').eq('selected', true);

    const items = (users || []).map(u => ({
      user_id: u.id,
      user_email: u.email,
      selection_count: (prefs || []).filter(p => p.user_id === u.id).length,
      product_count: (prefs || []).filter(p => p.user_id === u.id).length
    })).filter(x => x.selection_count > 0);
...
app.get('/admin/users/:id/product-preferences', auth, admin, async (req, res) => {
  try {
    const { data: prefs } = await supabase
      .from('user_product_selections')
      .select('*')
    ...
    const productIds = (prefs || []).map(p => p.catalog_product_id);
    const { data: products } = await supabase.from('catalog_products').select('*').in('id', productIds.length ? productIds : ['00000000-0000-0000-0000-000000000000']);
    const map = new Map((products || []).map(p => [p.id, p]));

    res.json({
      items: (prefs || []).map(p => ({ ...p, product: map.get(p.catalog_product_id) || {} })),
      countdown_selections: []
    });
  } catch(err) {
    console.error(err);
    res.status(500).json({ error: err.message });
...
app.get('/admin/countdowns/:id/users', auth, admin, async (req, res) => {
  res.json({ countdown: null, users: [] });
});
...
app.put('/admin/target-recommended-list-name', auth, admin, async (req, res) => {
  res.json({ success: true });
});
