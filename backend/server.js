...
function findDuplicateInSameGroup(
    ...
    const normalizedGroup = normalizeProfileAccountType(group);

    // Premium Bandai permits profiles that share profile names, login emails,
    // shipping details, phone numbers, and payment cards. Duplicate protection
    // remains enabled for every other store.
    if (normalizedGroup === "bandai") {
        return null;
    }
    ...
    for (const profile of profiles) {
        ...
        if (normalizeProfileAccountType(profile.account_type) !== normalizedGroup) {
            continue;
        }
...
app.post("/profiles/import", auth, async (req, res) => {
    try {
        ...
        for (const entry of rawProfiles) {
            ...
            if (!skipDuplicateProtection) seen.add(dedupeKey);
