...
function render(){const q=$('searchOrders').value.toLowerCase();const selectedStatus=$('statusFilter').value;const selectedYear=$('yearFilter').value;const rows=allOrders.filter(o=>{const matchesSearch=`${o.store} ${o.order_number} ${o.product_summary} ${o.source_email||''}`.toLowerCase().includes(q);const matchesStatus=!selectedStatus||String(o.status||'')===selectedStatus;const matchesYear=!selectedYear||String(new Date(o.order_date||0).getFullYear())===selectedYear;return matchesSearch&&matchesStatus&&matchesYear});$('ordersList').innerHTML=rows.length?rows.map(o=>`<article class="order-card"><div class="order-head"><div><span class="status-pill status-${esc(o.status)}">${esc(o.status)}</span><h3>${esc(o.store).toUpperCase()} · ${esc(o.order_number)}</h3><p>${esc(o.product_summary||'Product details will improve as receipt emails are parsed.')}</p><p class="order-email"><b>Purchase email:</b> ${esc(o.source_email||'Not linked yet')}</p></div><strong>${money(o.total)}</strong></div><div class="order-meta"><div><small>Order date</small><br><b>${o.order_date?new Date(o.order_date).toLocaleDateString():'—'}</b></div><div><small>Tracking</small><br><b>${esc(o.tracking_number||'—')}</b></div><div><small>Credits spent</small><br><b>${money(o.credits_spent)}</b></div><div><small>Last update</small><br><b>${o.last_status_at?new Date(o.last_status_at).toLocaleString():'—'}</b></div></div><div class="order-actions"><button class="btn" onclick="openReceipt('${o.id}')">View / Print Receipt</button><button class="btn" onclick="editOrder('${o.id}')">Edit</button><button class="btn btn-danger" onclick="deleteOrder('${o.id}')">Delete</button></div></article>`).join(''):'<section class="tracker-panel"><p>No tracked orders match this view yet.</p></section>'}
