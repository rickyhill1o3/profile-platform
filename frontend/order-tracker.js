...
function statusCardClass(o){const s=String(o.status||'waiting_confirmation');if(s==='canceled'||s==='refunded')return 'order-canceled';if(s==='delivered')return 'order-delivered';if(s==='shipped')return 'order-shipped';if(s==='confirmed'||s==='processing')return 'order-confirmed';if(!o.has_linked_email)return 'order-waiting';return 'order-waiting'}
...
function renderItems(o){
  ...
  return `<div class="order-items">${items.map(i=>`<div class="order-item-row"><div><b>${esc(i.product_name||'Item')}</b>${i.size?`<small>Size: ${esc(i.size)}</small>`:''}${i.style?`<small>Style: ${esc(i.style)}</small>`:''}</div><span class="item-role ${i.role==='main'?'item-main':i.role==='filler'?'item-filler':''}">${esc(i.role||'item')}</span><span class="item-status">${esc(i.status||'confirmed')}</span></div>`).join('')}</div>`
}
...
function render(){const q=$('searchOrders').value.toLowerCase();const selectedStatus=$('statusFilter').value;const selectedYear=$('yearFilter').value;const rows=allOrders.filter(o=>{const matchesSearch=`${o.store} ${o.order_number} ${o.product_summary} ${o.source_email||''} ${(o.items||[]).map(i=>i.product_name).join(' ')}`.toLowerCase().includes(q);const matchesStatus=!selectedStatus||String(o.status||'')===selectedStatus;const matchesYear=!selectedYear||String(new Date(o.order_date||0).getFullYear())===selectedYear;return matchesSearch&&matchesStatus&&matchesYear});$('ordersList').innerHTML=rows.length?rows.map(o=>`<article class="order-card ${statusCardClass(o)}"><div class="order-head"><div><span class="status-pill status-${esc(o.status)}">${esc(o.status)}</span>${!o.has_linked_email&&!['shipped','delivered'].includes(String(o.status||''))?'<span class="email-missing-pill">No retailer email linked</span>':''}<h3>${esc(o.store).toUpperCase()} · ${esc(o.order_number)}</h3><p>${esc(o.product_summary||'Product details will improve as receipt emails are parsed.')}</p><p class="order-email"><b>Purchase email:</b> ${esc(o.source_email||'Not linked yet')}</p></div><strong>${esc(orderHeadlineValue(o))}</strong></div>${renderReconciliation(o)}${renderItems(o)}${renderShipments(o)}<div class="order-meta"><div><small>Order date</small><br><b>${o.order_date?new Date(o.order_date).toLocaleDateString():'—'}</b></div><div><small>Tracking</small><br><b>${esc((o.shipments||[]).length?`${o.shipments.length} package${o.shipments.length===1?'':'s'}`:(o.tracking_number||'—'))}</b>${!((o.shipments||[]).length)&&o.tracking_url?`<br><a class="tracking-link" href="${esc(o.tracking_url)}" target="_blank" rel="noopener">Track package</a>`:''}</div><div><small>${(['canceled','refunded'].includes(String(o.status||'').toLowerCase())&&o.credits_refunded)?'Credits refunded':'Credits spent'}</small><br><b>${(['canceled','refunded'].includes(String(o.status||'').toLowerCase())&&o.credits_refunded)?`Refunded ${money(o.credits_spent)}`:money(o.credits_spent)}</b></div><div><small>Last update</small><br><b>${o.last_status_at?new Date(o.last_status_at).toLocaleString():'—'}</b></div></div><div class="order-actions">${emailButtons(o)}<button class="btn" onclick="editOrder('${o.id}')">Edit</button><button class="btn btn-danger" onclick="deleteOrder('${o.id}')">Delete</button></div></article>`).join(''):'<section class="tracker-panel"><p>No tracked orders match this view yet.</p></section>'}
