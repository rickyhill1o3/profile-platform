...
async function createProductGroup() {
    const payload = {
        display_name: document.getElementById('groupDisplayName')?.value || '',
        category: document.getElementById('groupCategory')?.value || 'other',
        site_category: document.getElementById('groupCategory')?.value || 'other',
        shared_credit_cost: Number(document.getElementById('groupCredits')?.value || 0),
        site: 'target',
        is_active: true
    };
    ...
    document.getElementById('productGroupStatus').innerHTML =
        'Created Product Group: ' + (data.id || 'unknown');
}

async function searchGroupSku() {
    const sku = document.getElementById('groupSearchSku')?.value || '';

    const res = await fetch(API + '/catalog-products?search=' + encodeURIComponent(sku), {
        headers: authHeaders()
    });
    ...
    document.getElementById('groupSkuResults').innerHTML = html;
}

async function attachSkuToGroup() {
    const payload = {
        product_group_id: document.getElementById('attachGroupId')?.value,
        catalog_product_id: document.getElementById('attachCatalogId')?.value,
        is_primary: !!document.getElementById('attachPrimarySku')?.checked
    };

    const res = await fetch(API + '/admin/product-group-skus', {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify(payload)
    });
    ...
    document.getElementById('productGroupStatus').innerHTML =
        'Attached SKU to Group';
}

async function updateProductGroup() {
    const payload = {
        category: document.getElementById('editGroupCategory')?.value,
        site_category: document.getElementById('editGroupCategory')?.value,
        shared_credit_cost: Number(document.getElementById('editGroupCredits')?.value || 0)
    };
...
window.addEventListener('DOMContentLoaded', () => {
    document.getElementById('createProductGroupBtn')?.addEventListener('click', createProductGroup);
    document.getElementById('searchGroupSkuBtn')?.addEventListener('click', searchGroupSku);
    document.getElementById('attachSkuToGroupBtn')?.addEventListener('click', attachSkuToGroup);
    document.getElementById('updateGroupBtn')?.addEventListener('click', updateProductGroup);
});
