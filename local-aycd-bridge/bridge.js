...
function sanitize(cfg={}){const requestedHost=clean(cfg.host)||'127.0.0.1';return {host:['localhost','127.0.0.1','::1'].includes(requestedHost)?'127.0.0.1':requestedHost,port:Number(cfg.port||43283),username:clean(cfg.username)||'inbox@aycd.me',password:clean(cfg.password),secure:!!cfg.secure,lookbackDays:Math.max(1,Math.min(90,Number(cfg.lookbackDays||30)))}}
...
app.get('/',(req,res)=>{res.type('html').send(`<!doctype html><meta charset="utf-8"><title>Shore Shack AYCD Helper</title><style>body{font-family:Arial,sans-serif;background:#0f172a;color:#fff;display:grid;place-items:center;min-height:100vh;margin:0}.card{background:#fff;color:#0f172a;padding:28px;border-radius:18px;max-width:620px;box-shadow:0 20px 60px #0008}.ok{color:#15803d;font-weight:800}</style><div class="card"><h1>The Shore Shack AYCD Helper</h1><p class="ok">Running on this laptop</p><p>Keep this command window and AYCD Inbox open. Return to Order Tracker and click <b>Test</b>.</p><p>This helper only listens on this computer at port ${PORT}.</p></div>`)});
...
app.listen(PORT,'127.0.0.1',()=>{console.log('');console.log('============================================================');console.log(' The Shore Shack AYCD bridge is RUNNING');console.log(` Health page: http://127.0.0.1:${PORT}`);console.log(' Keep this window open while using AYCD Order Tracker scans.');console.log('============================================================');console.log('')});
