...
function browserlessEndpointWithLaunchOptions(endpoint, normalized, proxy, log = () => {}) {
  ...
  // Browserless rejects timeout values above 60000 on most shared plans.
  // The app env is named ORDER_RECHECK_TIMEOUT_MS, but Browserless' `timeout`
  // query parameter also accepts millisecond-style values on hosted endpoints.
  // Clamp aggressively so a stale Render env like 90000 never causes a 400.
  ...
  const extra = [
    ...
    `timeout=${encodeURIComponent(String(browserlessTimeout))}`,
